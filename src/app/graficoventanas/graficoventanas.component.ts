...
@Component({
  selector: 'app-graficoventanas',
  standalone: true,
  imports: [AgChartsModule, HttpClientModule],
  template: `<ag-charts [options]="chartOptions"></ag-charts>`,
  styleUrls:['./graficoventanas.component.css'],
})
export class GraficoventanasComponent implements OnInit {
  ...
  ngOnInit() {
    this.http.get<any[]>('https://localhost:7114/ControllerLogsPrincipal/vistas-por-dia')
      .subscribe(data => {
        ...
        console.log('Opciones del gráfico después de actualizar:', this.chartOptions);
      });
  }
}
